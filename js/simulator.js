...
var Simulator = function () {
        ...
        for (i in initStates) {
            ...
            for (j = 0;; j++) {
                ...
                if (node !== null) {
                    ...
                    if (prev.length > 0) doc.createArc(prev + '->' + map).type(sb.ArcType.PositiveInfluence).source(prev).target(map);
                    break;
                } else {
    ...
    this.exportRBoolNet = function () {
        ...
        // rule
        for (i in network.rules) {
            r = network.rules[i].replace(/&&/g, '&').replace(/\|\|/g, '|')
    ...
    this.exportPythonBooleanNet = function () {
        ...
        // rule
        for (i in network.rules) {
            r = network.rules[i].replace(/&&/g, 'and').replace(/\|\|/g, 'or')
                .replace(/true/g, 'True').replace(/false/g, 'False')
                .replace(/[!]/g, 'not');
            pbn += i + '* = ' + r + '\n';
        }
    ...
    this.start = function () {
        ...
        $('#buttonSimulate').button("option", "icons", {
