...
var Simulator = function () {
	 ...
	 */
	var updateNodeColor = function (nodeid) {
		var opacity = 0;
	 ...
	 */
	var onEditRuleDialogOpen = function (event) {
	 ...
	 */
	var onEditRuleDialogSave = function () {
		var rule = $('#textRule')
			.val();
		var id = $('#textID')
			.text();
	 ...
	 */
	var onNodeClick = function (event) {
	 ...
	 * of a node.
	 */
	var showRuleBox = function (event) {
		var id = $(this)
			.attr('id');
...
//		var menuHeight = 41;   // the height of <ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
	 ...
	 * @param {Object} state The state of the network.
	 */
	var upateRickshawTimeseries = function (nodes, state) {
		var i, id;
	...
	this.run = function () {
		...
		upateRickshawTimeseries(network.nodes, network.state);
		$('#textIteration')
	...
	this.destroy = function () {
		$('#buttonSimulate')
			.unbind('click', obj.start);
		$('#buttonAnalyse')
			.unbind('click', obj.findAttractors);
	};
};